...
def load_model(model, material):
    mdl = LossModel(material, team=model.lower())
    return mdl
...
def make_donut(input_response, input_text, input_color, range=[50, 450]):
    ...
    source = DataFrame({"Topic": ["", input_text], "% value": [100 - ratio, ratio]})
    source_bg = DataFrame({"Topic": ["", input_text], "% value": [100, 0]})

    plot = (
        alt.Chart(source)
        .mark_arc(innerRadius=40, cornerRadius=22)
        .encode(
            theta="% value",
            color=alt.Color(
                "Topic:N",
    ...
    plot_bg = (
        ...
        .encode(
            ...
            color=alt.Color(
                "Topic:N",
                scale=alt.Scale(domain=[input_text, ""], range=chart_color),  # 31333F
                legend=None,
            ),
        )
        .properties(width=128, height=128)
...
def generateTriSequence(time, amplitude, phase, duty):
    ...
    for i in range(len(time)):
        if duty + phase / 360 <= 1:
            if time[i] <= phase / 360:
                yData.append(
                    -amplitude + (phase / 360 - time[i]) * (2 * amplitude) / (1 - duty)
                )
            elif time[i] <= phase / 360 + duty:
                yData.append(
                    -amplitude + (time[i] - phase / 360) * (2 * amplitude) / duty
                )
            else:
                yData.append(
                    amplitude
                    - (time[i] - phase / 360 - duty) * (2 * amplitude) / (1 - duty)
                )
            ...
            if time[i] <= phase / 360 - (1 - duty):
                yData.append(
                    amplitude
                    - (phase / 360 - (1 - duty) - time[i]) * (2 * amplitude) / duty
                )
            elif time[i] <= phase / 360:
                yData.append(
                    amplitude
                    - (time[i] - (phase / 360 - (1 - duty)))
                    * (2 * amplitude)
                    / (1 - duty)
                )
            else:
    ...
    for i in range(len(time)):
        if phase / 360 <= (1 - duty1 - duty2) / 2:
            if time[i] <= phase / 360:
                ...
                yData.append(
                    -amplitude + (time[i] - phase / 360) * (2 * amplitude) / duty1
                )
                ...
                yData.append(
                    amplitude
                    - (time[i] - (phase / 360 + duty1 + (1 - duty1 - duty2) / 2))
                    * (2 * amplitude)
                    / duty2
                )
            else:
            ...
            if time[i] <= phase / 360 - ((1 - duty1 - duty2) / 2):
                yData.append(
                    -amplitude
                    + ((phase / 360 - (1 - duty1 - duty2) / 2) - time[i])
                    * (2 * amplitude)
                    / duty2
                )
            elif time[i] <= phase / 360:
            ...
            if time[i] <= phase / 360 - (1 - duty1 - duty2) / 2 - duty2:
                ...
                yData.append(
                    amplitude
                    - (time[i] - (phase / 360 - (1 - duty1 - duty2) / 2 - duty2))
                    * (2 * amplitude)
                    / duty2
                )
            elif time[i] <= phase / 360:
            ...
            if time[i] <= phase / 360 - (1 - duty1 - duty2) - duty2:
                yData.append(
                    amplitude
                    - ((phase / 360 - (1 - duty1 - duty2) - duty2) - time[i])
                    * (2 * amplitude)
                    / duty1
                )
...
def footer():
    ...
    myargs = [
        ...
        link(
            ...
            image(
                "https://lh3.ggpht.com/_uP6bUdDOWGS6ICpMH7dBAy5LllYc_bBjjXI730L3FQ64uS1q4WltHnse7rgpKiInog2LYM1",
                width=px(19),
                height=px(19),
                margin="0em",
                align="top",
...
def main():
    ...
    # fmt: off
    materials = ["3C90", "3C92", "3C94", "3C95", "3E6", "3F4", "77", "78", "79", "ML95S", "N27",
                 "N30", "N49", "N87", "T37", ] 
    # fmt: on
    resolution_params = {
        "Sydney": 128,
    ...
    with st.sidebar:
        ...
        st.image(
            "https://openday.sydney.edu.au/static/website/images/usyd-logo-white.png",
            caption="Powered by: Sydney University",
    ...
    # Define the layout
    col1, _, col2, _, col3, _ = st.columns(
        [1.2, 0.05, 1.3, 0.1, 1.2, 0.05]
    )  # First row
    st.markdown("<div style='height:0.5vh;'></div>", unsafe_allow_html=True)
    col4, _, col5, _, col6, _ = st.columns(
        [1.2, 0.05, 1.3, 0.1, 1.2, 0.05]
    )  # Second row
    ...
    with col1:
        st.write(
            """<span style='font-size: calc(0.6vw + 0.6vh + 10px); text-decoration: none;font-weight: bold;text-align: left;'> Excitation Waveform [B] </span>""",
            unsafe_allow_html=True,
        )
        ...
        for i, (label, icon_filename) in enumerate(shapes):
            with col1_1 if (i == 0 or i == 2) else col1_2:
                ...
                st.write(
                    f'<div style="text-align: left; margin-left:calc(40px - 1.5vw);"><span style="display: inline-block; width: calc(4vw + 25px); height: calc(4vh + 60px); fill: #F5F5F5;">{svg_code}</span></div>',
                    unsafe_allow_html=True,
                )
    ...
    with col3:
        ...
        st.write(
            """<span style='font-size: calc(0.6vw + 0.6vh + 10px); text-decoration: none;font-weight: bold;text-align: left;'> Excitation Parameters [B]</span>""",
            unsafe_allow_html=True,
        )
        ...
        if st.session_state["shape_id"] == 3:
            uploaded_file = st.file_uploader(
                "Resolution >= :red[" + str(resolution_params[model]) + " steps]",
                type=["csv"],
                help="Load user-defined flux excitation and :red[only read data in the 1st row].",
            )

    with col4:
        st.write(
            """<span style='font-size: calc(0.6vw + 0.6vh + 10px); text-decoration: none;font-weight: bold;text-align: left;'> Operating Condition [f, T]</span><div style='margin-bottom: 3vh;'></div>""",
            unsafe_allow_html=True,
        )
    ...
    with col2:
        ...
        st.write(
            """<span style='font-size: calc(0.6vw + 0.6vh + 10px); text-decoration: none;font-weight: bold;text-align: left;'> Time-Domain Response [B-H]</span>""",
            unsafe_allow_html=True,
        )
        ...
        if st.session_state["shape_id"] == 0:
            ...
            B = st.session_state["amplitude"] * sin(
                (360 * t + st.session_state["phase"]) * pi / 180
            )
        elif st.session_state["shape_id"] == 1:
            t = linspace(0, 1, resolution_params[model])
            B = array(
            ...
            if uploaded_file is not None:
                ...
                index = [
                    ...
                    for x in arange(
                        0,
                        len(first_row) - 1,
                        (len(first_row) - 1) / resolution_params[model],
                    )
                ]
        ...
        if sum(B) == 0:
            ...
            P, H = mdl(
                B / 1000, Frequency * 1000, Temperature
            )  # convert to B in [T], f in [Hz]
            H = H.squeeze()
        ...
        base = alt.Chart(df.reset_index()).encode(
            x=alt.X(
                "t",
                title="Fraction of a Cycle",
                axis=alt.Axis(titlePadding=14, titleFontWeight="bold"),
            )
        )
        chart = alt.layer(
            base.mark_line(strokeWidth=5, stroke="#8E2DE2").encode(
                y=alt.X(
                    "B",
                    title="B [mT]",
                    axis=alt.Axis(
            ...
            base.mark_line(strokeWidth=5, stroke="#E74C3C").encode(
                y=alt.X(
                    "H",
                    title="H [A/m]",
                    axis=alt.Axis(
    ...
    with col5:
        st.write(
            """<span style='font-size: calc(0.6vw + 0.6vh + 10px); text-decoration: none;font-weight: bold;text-align: left;'> Steady-State Loop [B-H]</span>""",
            unsafe_allow_html=True,
        )
        ...
        chart2 = (
            alt.Chart(df2)
            .mark_line(strokeWidth=5, stroke="#29b5e8")
            .encode(
                x=alt.X(
                    "H",
                    title="H [A/m]",
                    axis=alt.Axis(titlePadding=5, titleFontWeight="bold", grid=True),
                    sort=None,
                    scale=alt.Scale(
                        domain=(
                            round(min(H) - 0.05 * (max(H) - min(H))),
                            round(max(H) + 0.05 * (max(H) - min(H))),
                        )
                    ),
                ),
                y=alt.Y(
                    "B",
                    title="B [mT]",
                    axis=alt.Axis(titlePadding=0, titleFontWeight="bold", grid=True),
                ),
            )
    ...
    with col6:
        st.write(
            """<span style='font-size: calc(0.6vw + 0.6vh + 10px); text-decoration: none;font-weight: bold;text-align: left;'> Volumetric Loss [Pv]</span>""",
            unsafe_allow_html=True,
        )
        ...
        if sum(B) == 0:
            st.markdown(
                f'<div class="metric-container"><h1>Unknown</h1><p>kW/m³</p></div>',
                unsafe_allow_html=True,
            )
            ...
            st.markdown(
                f'<div class="metric-container"><h1>{round(P/1000,2)}</h1><p>kW/m³</p></div>',
                unsafe_allow_html=True,
            )
            ...
            with col6_1:
                ...
                st.download_button(
                    ...
                    label="Download 📂",
                    help="Download B-H-Pv in "
                    + str(resolution_params[model])
                    + " step as CSV",
